...
export function FinanceProvider({ children }: { children: ReactNode }) {
  ...
  const loadFromSupabase = async () => {
    ...
    try {
      ...
      if (periodsError) {
        console.error('[FinanceContext] Ошибка загрузки периодов:', periodsError);
    ...
    if (useSupabase && supabase) {
      const { error } = await supabase
        ...
        .update({
          ...
          is_for_special_employee: updatedExpense.isForSpecialEmployee,
          special_employee_id: updatedExpense.specialEmployeeId,
        })
        .eq('id', id);
  ...
  const createNewPeriod = async () => {
    ...
    const newPeriod: PayPeriod = {
      ...
      isClosed: false,
      calculation_version: 'v3' // НОВАЯ ЛОГИКА: 4 независимые кассы + вычет ЗП клоузеров с команд
    };
